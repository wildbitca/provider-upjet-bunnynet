...
PROJECT_NAME ?= provider-upjet-bunnynet
...
export TERRAFORM_PROVIDER_VERSION ?= 0.14.0
...
export TERRAFORM_NATIVE_PROVIDER_BINARY ?= terraform-provider-bunnynet_v0.14.0
...
xpkg.build.provider-upjet-bunnynet: do.build.images
...
$(if $(filter config,$(1)),provider-family-bunnynet,$(if $(filter monolith,$(1)),provider-upjet-bunnynet,provider-bunnynet-$(1)))
endef
...
build.family.image:
	...
	@cp cluster/images/provider-upjet-bunnynet/Dockerfile $(IMAGE_TEMP_DIR) || $(FAIL)
	@cp cluster/images/provider-upjet-bunnynet/terraformrc.hcl $(IMAGE_TEMP_DIR) || $(FAIL)
