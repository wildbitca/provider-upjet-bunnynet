...
import (
	...
	apisCluster "github.com/wildbitca/provider-upjet-bunnynet/apis/cluster"
	apisNamespaced "github.com/wildbitca/provider-upjet-bunnynet/apis/namespaced"
	"github.com/wildbitca/provider-upjet-bunnynet/config"
	"github.com/wildbitca/provider-upjet-bunnynet/internal/clients"
	controllerCluster "github.com/wildbitca/provider-upjet-bunnynet/internal/controller/cluster"
	controllerNamespaced "github.com/wildbitca/provider-upjet-bunnynet/internal/controller/namespaced"
	"github.com/wildbitca/provider-upjet-bunnynet/internal/features"
	"github.com/wildbitca/provider-upjet-bunnynet/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for UpjetBunnynet").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-upjet-bunnynet"))
	if *debug {
		ctrl.SetLogger(zl)
	}
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-upjet-bunnynet-monolith",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped UpjetBunnynet APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced UpjetBunnynet APIs to scheme")
	...
	if *enableChangeLogs {
		...
		clo := xpcontroller.ChangeLogOptions{
			ChangeLogger: managed.NewGRPCChangeLogger(
				...
				managed.WithProviderVersion(fmt.Sprintf("provider-upjet-bunnynet:%s", version.Version))),
		}
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated_monolith(mgr, clusterOpts), "Cannot setup cluster-scoped UpjetBunnynet controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated_monolith(mgr, namespacedOpts), "Cannot setup namespaced UpjetBunnynet controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup_monolith(mgr, clusterOpts), "Cannot setup cluster-scoped UpjetBunnynet controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup_monolith(mgr, namespacedOpts), "Cannot setup namespaced UpjetBunnynet controllers")
	}
