...
#   <apigroup>.upjet-bunnynet.upbound.io_<kind>.yaml   (cluster-scoped)
#   <apigroup>.upjet-bunnynet.m.upbound.io_<kind>.yaml  (namespaced)
#
# The sub-provider name matches the API group prefix (e.g., "dns" matches dns.upjet-bunnynet.*)
...
map_repo_name() {
    case "$1" in
    config) echo "provider-family-bunnynet" ;;
    monolith) echo "provider-bunnynet" ;;
    *) echo "provider-bunnynet-$1" ;;
    esac
}
...
if [ "${SUBPACKAGE}" = "monolith" ]; then
    ...
    # These are in the upjet-bunnynet.upbound.io and upjet-bunnynet.m.upbound.io groups directly
    for f in "${CRD_DIR}"/*.yaml; do
        basename=$(basename "$f")
        if echo "${basename}" | grep -qE '^upjet-bunnynet\.(m\.)?upbound\.io_'; then
            cp "$f" "${WORK_DIR}/crds/"
        fi
    done
else
    # Sub-provider gets only its API group CRDs
    # Match: <subpackage>.upjet-bunnynet.upbound.io_* and <subpackage>.upjet-bunnynet.m.upbound.io_*
    for f in "${CRD_DIR}/${SUBPACKAGE}.upjet-bunnynet."*.yaml; do
...
metadata:
  ...
  annotations:
    ...
    meta.crossplane.io/source: "https://github.com/wildbitca/provider-upjet-bunnynet"
...
if [ "${SUBPACKAGE}" != "monolith" ]; then
    cat >>"${WORK_DIR}/crossplane.yaml" <<YAML
  labels:
    pkg.crossplane.io/provider-family: provider-family-bunnynet
YAML
fi
